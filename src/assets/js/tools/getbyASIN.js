...
export default async function getbyASIN(arr, callback) {
  const combinedArr = [...new Set([...arr, ...arrGlobal])];
  arrGlobal.push(...arr);
...
//   async function processBatch(batch) {
//     async function onDataProcessed(asin, payload) {
//       processedASINs.add(asin);
//       callback({ asin, data: payload });
//       console.log({ asin, data: payload });
//     }
...
//   for (const batch of batches) {
//     if (totalRequests < 3 && processingASINs.size < 3) {
//       await processBatch(batch);
//     } else {
//       console.log("Maximum total requests reached. Adding to the queue.");
//       requestQueue.push(batch);
...
//   if (cachedResponse) {
//     const timestampedData = await cachedResponse.json();
