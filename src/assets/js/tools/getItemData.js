...
export default function getItemData(callback, find, current = document, isBackend) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      const result = await lazyFindElements(find, current, 8, 50);
      ...
      const bg_color = false ? await getMainImageBgColor(main_img) : null;
      let main_dimensions;
      if (!isBackend)
        main_dimensions = false ? await getImageDimensionsFromURL(hires_img, isBackend) : null;
